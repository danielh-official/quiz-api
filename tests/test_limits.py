...
def tree(db, user, root_limit=10, s_limit=2, t_limit=1):
...
def study_all(db, user, deck) -> list[int]:
...
def test_subdeck_limits_cap_their_subtree(db, user):
...
def test_own_limit_caps_deck_even_with_parent_room(db, user):
...
def test_parent_limit_caps_children(db, user):
    root, _, _ = tree(db, user, root_limit=1, s_limit=5, t_limit=5)
    assert len(study_all(db, user, root)) == 1


def test_limits_above_session_root_ignored(db, user):
